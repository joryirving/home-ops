...
function apply_config() {
    ...
    local -r cilium_config_dir="${ROOT_DIR}/kubernetes/${CLUSTER}/apps/kube-system/cilium/config"

    if [[ ! -d "${cilium_config_dir}" ]]; then
        log error "No Cilium config directory found" "directory=${cilium_config_dir}"
    fi

    if kubectl --context ${CLUSTER} --namespace kube-system diff --kustomize "${cilium_config_dir}" &>/dev/null; then
        log info "Cilium config is up-to-date"
    else
        if kubectl --context ${CLUSTER} apply --namespace kube-system --server-side --field-manager kustomize-controller --kustomize "${cilium_config_dir}" &>/dev/null; then
